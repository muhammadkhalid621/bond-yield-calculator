...
function priceFromPeriodicYield(
  ...
  if (Math.abs(periodicYield) < 1e-12) {
...
function solvePeriodicYtm(
  ...
  const initialGuess = Math.max(-0.95, couponRateDecimal / frequencyPerYear);
  let x = initialGuess;
  ...
  for (let i = 0; i < 50; i += 1) {
    ...
    const next = x - fx / dfx;
    if (!Number.isFinite(next) || next <= -0.999999 || next > 10) {
      break;
    }
...
export function validateBondInput(input: BondCalculationInput): string[] {
  ...
  if (!Number.isFinite(input.faceValue) || input.faceValue <= 0) {
    errors.push('Face value must be a positive number.');
  }
  ...
  if (!Number.isFinite(input.yearsToMaturity) || input.yearsToMaturity <= 0) {
    errors.push('Years to maturity must be a positive number.');
  }
...
export function calculateBondMetrics(input: BondCalculationInput): BondCalculationResult {
  ...
  if (totalPeriods <= 0) {
    throw new Error('Total periods must be at least 1.');
  }
  ...
  const ytmPct = periodicYtm * frequencyPerYear * 100;

  const totalInterestEarned = couponPayment * totalPeriods;
  const premiumDiscountAmount = input.marketPrice - input.faceValue;
  const tradingStatus =
    premiumDiscountAmount > 0 ? 'premium' : premiumDiscountAmount < 0 ? 'discount' : 'par';
